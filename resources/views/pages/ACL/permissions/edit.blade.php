        ...
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between my-4 px-4">
                <div class="flex">
                    ...
                    <h3 class="text-base font-medium text-gray-800 dark:text-white/90 mb-2 sm:mb-0">
                        Permissão de acesso - atualização
                    </h3>
                </div>
            </div>
            <div class="border-t border-gray-100 p-5 dark:border-gray-800 sm:p-6">
                ...
                <div x-data="dataTable()"
                    ...
                    <!-- Formulario -->
                    <form action="{{ route('permissions.update', $permission) }}" method="POST" class="space-y-6 m-6">
                        @csrf
                        @method('PUT')
                        <!-- Campo Nome -->
                        <div>
                            <x-input-label for="name" :value="__('Name')" />
                            <x-text-input id="name" name="name" type="text" value="{{ $permission->name }}" required autofocus
