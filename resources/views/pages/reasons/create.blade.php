    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                ...
                <form action="{{ route('reasons.store') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        {{-- <label for="purge" class="block text-sm font-medium text-gray-700 mb-2">Expurga?</label> --}}
                        <label id="purge" class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" id="purge" name="purge" value="1"
                                class="sr-only peer" {{ old('purge', $model->purge ?? false) ? 'checked' : '' }}>

                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 
                                        peer-focus:ring-cyan-300 rounded-full peer 
...
const checkbox = document.getElementById('purge_checkbox');
const hiddenInput = document.getElementById('purge_hidden');
